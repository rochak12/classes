 ...
 *    Rochak Kunwar
 * Summary:
 *    The Lab was to do matrix multiplication using threads.
 *    Each thread was working independently and they/threads collected its
 *    results at the end.
 ************************************************************************/
...
struct Combination      //This is for defining the location
{                       //in the destination matrix
	int row;             // 1,2 means 2nd row 3rd column
	int column;
	int count;
};
...
void* Operation(void* p);        //Declaration of a function where I do my calculation
...
bool multiply(MatrixMultiplication& m)
{
	...
	int size = numRowDes * numColDes;
	pthread_t threads[size];

	int count = 0;
	for (int roww = 0; roww < numRowDes; roww++)
		for (int col = 0; col < numColDes; col++)
		{
			Combination* combination = new Combination;     //making instance of combination
			combination->row = roww;                        //to pass over thread
			combination->column = col;
			...
			pthread_create(
				...
				NULL,
				Operation,
				(void*)combination                       //changing to type *Void 
			);
			count++;
		}
	...
	for (int row = 0; row < numRowDes; row++)
		for (int col = 0; col < numColDes; col++)
		{
			pthread_join(threads[count], NULL);           //Collecting all the threads
			count++;
		}
	return true;
}

/*Matrix row and column Operations happens here*/
void* Operation(void* p)
{
	const Combination* tempCombination = (const Combination*)p;
	int row = (*tempCombination).row;
	int col = (*tempCombination).column;

	int sum = 0;
	for (int i = 0; i < matrix.numColLHS; i++)
	{
		sum += matrix.arrayLHS[row * matrix.numColLHS + i] *
			matrix.arrayRHS[i * matrix.numColRHS + col];
	}
	matrix.arrayDestination[(*tempCombination).count] = sum;
	pthread_exit(NULL);
}
