 ...
 *    Br. Helfrich
 * Summary:
 *    This example shows how to use mutex locks with pthreads
 *
...
void * thread(void * p)      // accepting no parameters
{
   while (!done)
   {
      ...
      // critical section
      pthread_mutex_lock(&lock);     // try commenting this out ...
      cerr << "Start(" << (char *)p << ")...";
      usleep(rand() % 150000);
      cerr << "...Finish(" << (char *)p << ")\n";
      pthread_mutex_unlock(&lock);   // ... and this out also
   }
