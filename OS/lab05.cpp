...
void * solveForDestinationCell(void *p)
{
   const threadInput *input = (const threadInput *)p;

   int answer = 0;
   for (int i = 0; i < matrix.numColLHS; i++)
   {
      answer += matrix.arrayLHS[input->row_LHS * matrix.numColLHS + i] * 
                matrix.arrayRHS[i * matrix.numColRHS + input->col_RHS];
   }

   matrix.arrayDestination[input->row_LHS * matrix.numColRHS + input->col_RHS] = answer;
   delete input;
...
bool multiply(MatrixMultiplication & m)
{
   ...
   for (int row = 0; row < numRowDes; row++)
   {
      for (int col = 0; col < numColDes; col++)
      {
         ...
         pthread_t newThread;
         pthread_create(&newThread, // a pointer to the thread created
                        NULL,         // use the default set of attributes
                        solveForDestinationCell, // the function to be called
                        (void *) input);
